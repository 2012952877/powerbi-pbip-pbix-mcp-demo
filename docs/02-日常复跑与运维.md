...
PBIP 转 PBIX 时，逐文件精确匹配的内置 `Synthetic`/`Rich` 样例可自动刷新；其他工程须在所引用的语义模型目录中包含非空 `.pbi/cache.abf`。rc7 对 ZIP、文件夹及 MCP 使用同一入队前规则：缺缓存返回 `DATA_CACHE_REQUIRED`，不创建任务、不保留上传文件、不增加“我的任务”或队列条目，也不启动 Desktop。需要数据源登录或刷新时，先在批准的 Desktop 环境人工处理，再提供完整工程，不在网页中上传数据库密码。

definitions 的新进程复开不会刷新数据。关闭时最多处理两个不同的、已识别且属于该任务的保存决定窗口，每个窗口只丢弃一次；第三个、未知或保护类提示仍会失败。不要通过人工点击这些窗口让任务看似完成，故障原因应保留在本任务的私有诊断中。

## 3. 提交后沿自己的任务查看结果

确认输入、目标与数据模式后，点击一次“提交转换”。请求成功后，“我的任务”出现该任务；多个用户可以同时排队，但一个 worker 只执行一个任务。状态自动更新，也可用“刷新”读取服务端结果。

2026-09-14 核对的演示版本已包含提交保护；其他部署需安装对应更新。节点明确不可用时，先恢复节点；若仍要离线排队，必须确认页面显示的有限期限和超时风险。默认不勾选，状态未知或连接失败时不允许继续提交。`ready=true` 的忙碌节点仍能接收排队任务。已提交的任务仍可查询，queued 仍可取消，成功产物仍可下载。
...
部署命名适配版本后，新任务会自动沿用上传文件或工程文件夹的名称主体：如 `客户销售.pbix`、`客户销售.pbip.zip` 和 `客户销售.verification.json`。旧任务的固定名称不变；浏览器遇到本地同名文件可能自行添加序号，这与服务端改名不同。命名适配不改变 ZIP 内部项目引用，CLI 显式指定的输出路径也保持不变。
...
| `QUEUE_TIMEOUT` | 该任务已在领取前超时，不再等待它自动变成功；不据此认定源文件损坏 | 查交互会话、worker 退出原因及队列积压，恢复后再新建任务 |
| `INTERACTIVE_SESSION_LOST` | 本次转换因桌面断连或锁定中止；保留原任务 | 先恢复专用桌面；SessionAware 等待中的 worker 会恢复接单，Manual 已退出时再显式 Start；原失败任务须新建重试 |
| `DATA_CACHE_REQUIRED` / `PORTABLE_CACHE_MISSING` | 缺缓存的正向提交在入队前被拒绝；portable 导出失败则查任务详情 | 作者处理数据加载；不借其他任务缓存补齐 |
| `DESKTOP_UNEXPECTED_SAVE_PROMPT` | 不到远端替 worker 点击未知窗口；联系管理员 | 核对 rc7 有界双保存提示修复是否安装；仍失败则按本任务诊断检查实际窗口，不自动确认保护类提示 |
...
每次查看 VM 的实际电源、Windows 用户会话和 worker 心跳。当前 rc7 / 0.2.4 部署已把 worker 配置为 `IdleTimeout=0`、计划任务 `ExecutionTimeLimit=PT0S`，保留 `ReviewSeconds=20`，不再因空闲 30 分钟或运行两小时而退出；新注册但未配置的任务仍保留有限期限默认值。RDP 必须持续 **Active 且未锁定**，单次转换仍受 600 秒监督；没有自动登录、解锁或守护重启。

VM 自动关机沿用既有 Disabled 配置，本次升级不改停机策略。持续运行的 VM、Bastion、App Service Plan 和保留存储均需预算，任何停机策略调整都应与用户任务及下载协调。
...
执行位置：**管理员本机 PowerShell 终端 B**。这里使用 0.2.4 部署工作目录，不在本资料夹创建 `.venv`。如果只拿到了包，先按完整指南将[rc7 / 0.2.4 源码与部署入口](../README.md)解压部署一次；不要每天重装或退回旧候选版本。
...
继续提交要看 **worker.ready=true**，不是仅看计划任务 Running。Doctor 中的 `caller_session` 指管理命令所在会话，经 SSH 调用时为 0 是正常的；实际 worker 应位于已连接解锁的普通用户交互会话。ready 表示可以接单，busy 表示已有任务在执行，新任务会排队。Start 幂等复用同一任务并保留已注册参数；现有演示环境使用 `ReviewSeconds=20`、`IdleTimeout=0`，不要在日常启动时重新 Register 或改动它。

心跳过期只说明当前无法确认 worker 可用，不足以区分锁屏、RDP 断连、进程退出或 VM 故障。新源码会在 `last_reported_state`、`last_reported_message` 中保留最后一次报告，并显示 `updated_at`；这些都是历史诊断，不能当作当前 ready。若仍不明确，在获准的管理通道中只读核对具名任务、`C:\PBIPMCP\runs\worker-*` 的最后一次运行日志和用户会话，再决定恢复操作。`IdleTimeout=0` 只避免空闲退出，不会在断连后替用户登录、解锁或重新启动。

2026-09-14 已核对演示部署启用了 `SessionAware`；其他环境需另行部署和显式启用，旧 `Manual` 模式默认不变。`waiting_for_session` 表示 worker 进程仍在等待而非已退出：只有桌面正常恢复连接、解锁且探测通过后才自动接单。不要把它当成 ready，不需要因为它仍是 Running 就重复启动；队列的原截止时间不会延长。状态、私有日志提供故障提示，没有自动发送对外通知。

以下是**仅供切换空闲策略的维护分支**，本次部署已经完成，不是每日启动步骤。先协调全局队列及下载/传输静止，在 VM 管理 PowerShell 安全停止已有 worker；只有任务为 Ready、没有运行中的队列任务且 worker 不为 ready 时，Configure 才允许更新：

```powershell
$WorkerManage = 'C:\PBIPMCP\app\scripts\manage-worker.ps1'
& $WorkerManage -Action Stop -WaitSeconds 700
& $WorkerManage -Action Configure -IdleTimeout 0
& $WorkerManage -Action Start
& $WorkerManage -Action Status
```

不传 `ReviewSeconds` 会保留原值；不要用重新 Register 覆盖已有任务。核对唯一 action、工作目录、`Interactive + Limited`、`idle_timeout=0` 和 `PT0S`，并重新确认 RDP Active/未锁定。Stop 仍等当前转换完成后退出，不强杀或删除任务，也不会被当成 Configure/Start 自动重复执行。
...
以下是**管理员已批准并完成代码升级后的维护分支**，不是网页用户步骤。先确认上传、下载和全局队列静止，备份代码、队列及现有任务 XML，保留专用用户与原有 portal 启动参数；然后在 VM 管理终端执行：

```powershell
$WorkerManage = 'C:\PBIPMCP\app\scripts\manage-worker.ps1'
& $WorkerManage -Action Stop -WaitSeconds 700
& $WorkerManage -Action Configure -IdleTimeout 0 -RecoveryMode SessionAware
& $WorkerManage -Action Start -WaitSeconds 60
& $WorkerManage -Action Status
```

不传 `ReviewSeconds` 保留原值。SessionAware 的任务配置应为 Interactive + Limited、IgnoreNew、`PT0S`、三次失败重启/一分钟间隔，以及**仅专用用户**的登录触发；没有自动登录、自动解锁、无条件开机执行 GUI，也不关闭锁屏/NLA/超时安全策略。正常 `Start` 可以报告“已启动但在等待桌面”，此时 `worker.ready` 仍为 false，不能宣布转换服务可用。

`Stop` 即使任务已退出或正在等待重启，也会为已有自有任务写持久 `worker.stop`。定时失败重启和下次登录触发都不会清除它，只有显式 `Start` 才清除；禁用任务需管理员另行启用。恢复模式不能让已经失败的客户任务自动成功或重跑。要撤回此模式，安全 Stop 后执行 `Configure -RecoveryMode Manual`，保留 Idle/Review 参数，再按批准的桌面前提 Start。

真正的上线验收应在无客户活动的专用窗口完成：无任务时断开 RDP，确认心跳变为等待且不接单；恢复连接并解锁，确认无需重启 worker 即重新 ready，并完成一个新合成任务；显式 Stop 后观察失败重启/登录触发不会恢复接单。VM 重启后无人登录时应保持不可用，而不是假报绿色。转换中的故障注入必须单独批准，不能拿客户任务做试验。未执行这些实际场景前，只能称“源码提供恢复机制”，不能称“24 小时无人值守已验证”。
...
| queued | 在 `queue_deadline` 前等待可用 worker；明确放弃时才能用 cancel，领取后取消可能被拒绝 |
| QUEUE_TIMEOUT | 领取前的排队期限已过；检查当时的节点状态和队列积压，恢复后另建任务，不延长旧任务或伪造成功 |
| INTERACTIVE_SESSION_LOST | 当前转换因会话断连或锁定失败；恢复 Active、解锁的专用桌面，核对 worker 后另建任务 |
