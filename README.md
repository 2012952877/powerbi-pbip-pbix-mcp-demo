...
**definitions 不等于脱敏**：M、TMDL、参数及连接定义仍可能有内嵌数据或敏感内容；portable 还携带模型缓存。非内置 PBIP 工程不自动刷新，所引用语义模型目录缺少非空 `.pbi/cache.abf` 时，在创建任务、写入队列和保留上传文件之前返回 `DATA_CACHE_REQUIRED`；只有逐文件精确匹配的内置样例可例外。

definitions 导出仍经过真实 Save As、剔除缓存与本机设置、独立副本新进程复开。关闭该复开进程时，仅对本任务拥有且已识别的保存决定窗口有界响应：最多两个不同窗口，每个仅丢弃一次；未知登录、安全、法律或保护提示不自动绕过。凭据、用户库、队列、运行诊断及内部部署回执不进入仓库。
